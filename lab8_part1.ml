  ...
  struct
    ...
    let intersect (intvl1 : interval) (intvl2 : interval) : interval =
      ...
      | Interval (a, b), Interval (x, y) -> 
        if Endpoint.compare b x <= 0 || Endpoint.compare y a <= 0 then Empty
          else let lower = if Endpoint.compare a x > 0 then a else x in
            let upper = if Endpoint.compare b y < 0 then b else y in
...
let intvl1 = IntInterval.create 1 8 ;;
